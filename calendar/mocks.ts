...
import { ENTITIES, getEntityColor } from "@/calendar/entities";
...
const mockGenerator = (numberOfEvents: number): IEvent[] => {
  ...
  const currentEventEntity = ENTITIES[Math.floor(Math.random() * ENTITIES.length)];
  const currentEvent = {
    id: currentId++,
    ...
    color: getEntityColor(currentEventEntity.name),
    description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
    entity: currentEventEntity.name,
  };
  ...
  while (i < numberOfEvents - 1 && attempts < maxAttempts) {
    ...
    const eventEntity = ENTITIES[Math.floor(Math.random() * ENTITIES.length)];
    result.push({
      id: currentId++,
      startDate: startDate.toISOString(),
      endDate: endDate.toISOString(),
      title: EVENTS[Math.floor(Math.random() * EVENTS.length)],
      color: getEntityColor(eventEntity.name),
      description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
      entity: eventEntity.name,
    });
